...
<div class="p-4 lg:p-6 max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <!-- Engagement Stats -->
        <div class="px-4 py-3 bg-gradient-to-r from-green-50 to-emerald-50 border-t border-gray-100 flex items-center justify-between text-sm">
            ...
            <div class="relative" x-data="{ shareOpen: false }" onclick="event.stopPropagation();">
                <button @click.prevent="shareOpen = !shareOpen" @click.away="shareOpen = false" class="flex items-center gap-1.5 cursor-pointer text-gray-500 hover:text-green-600 hover:bg-green-50 px-3 py-1.5 -mr-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-green-200">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg>
                    <span class="font-bold">Share</span>
                </button>

                <!-- Dropdown Menu -->
                <div x-show="shareOpen" style="display: none;" x-transition.opacity.duration.200ms class="absolute right-0 bottom-full mb-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50">
                    ...
                    <button @click.prevent="navigator.clipboard.writeText('{{ route('feed.show', $feed->id) }}'); alert('Link copied to clipboard!'); shareOpen = false;" class="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 transition-colors text-left">
                    ...
                    <button @click.prevent="if(navigator.share) { navigator.share({ title: 'Eco Platform', url: '{{ route('feed.show', $feed->id) }}' }) } else { alert('Web Share API is not supported in your browser.'); }; shareOpen = false;" class="w-full flex items-center gap-3 px-4 py-2 hover:bg-gray-50 transition-colors text-left">
                        <div class="w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center flex-shrink-0 text-gray-600">
