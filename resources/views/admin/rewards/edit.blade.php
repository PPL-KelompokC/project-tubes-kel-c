...
@section('page_title', 'Edit Reward')
@section('page_subtitle', 'Modify existing reward details.')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8" x-data="rewardForm()">
    ...
    <div class="lg:col-span-2">
        <div class="bg-white rounded-2xl border border-slate-200 card-shadow overflow-hidden">
            <form action="{{ route('admin.rewards.update', $reward) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PUT')
                <div class="space-y-6">
                    <!-- Basic Info -->
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            <label class="text-sm font-bold text-slate-700">Update Image</label>
...
<script>
    function rewardForm() {
        return {
            name: @json($reward->name),
            category: @json($reward->category),
            description: @json($reward->description),
            points: @json($reward->points_required),
            stock: @json($reward->stock),
            status: @json($reward->status),
            imagePreview: @json($reward->image ? Storage::url($reward->image) : null),
            handleImage(e) {
